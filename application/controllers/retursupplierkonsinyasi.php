...
class Retursupplierkonsinyasi extends CI_Controller {
	public function __construct(){
		parent::__construct();
		
		$this->load->model('user_model');
		$this->load->model('retursupplierkonsinyasi_model');
	...
	public function index(){
		...
		$this->load->view('retursupplierkonsinyasi/home');
		$this->load->view('general/footer');
	}
	
	public function getdataretursupplier(){
		$DataReturSupplier = $this->retursupplierkonsinyasi_model->getDataReturSupplier($_POST);
	...
	public function listpilihretursupplier(){
		$DataReturSupplier = $this->retursupplierkonsinyasi_model->getDataListReturSupplier($_POST);
	...
	public function simpanretursupplier(){
		if($_POST['kwt'] > 0){
			...
			$this->retursupplierkonsinyasi_model->SimpanReturSupplier($_POST);
	...
	public function hapusretursupplier(){
		$this->retursupplierkonsinyasi_model->HapusReturSupplier($_POST);
	}

	public function syncstokretursupplier(){
		$DataRetur = $this->retursupplierkonsinyasi_model->getAllDataRetur($_POST);
	...
	public function getdatabarangretursupplier(){
		$DataBarangot = $this->retursupplierkonsinyasi_model->getDataBarangReturSupplier($_POST);
	...
	public function getdatabarangsupplier(){
		$DataBarangSupplier = $this->retursupplierkonsinyasi_model->getDataBarangSupplier($_POST);
	...
	public function getrekapretursupplier(){
		$DataReturSupplier = $this->retursupplierkonsinyasi_model->getRekapReturSupplier($_POST);
	...
	public function cetakretursupplier(){
		$DataBarangReturSupplier = $this->retursupplierkonsinyasi_model->getDataBarangReturSupplier($_GET);
		...
		$html .= "<table>
					<tr>
						...
						<td align=\"right\">Retur Supplier Konsinyasi</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
		...
		$html .= "<table>
					...
					<tr>
						...
						<td width=\"30\"> : </td>
						<td width=\"200\">Retur Supplier Konsinyasi</td>
						<td width=\"70\"></td>
		...
		$pdf->Output('ReturSupplierKonsinyasi.pdf', 'I');
	}
	...
	public function getrekapharian(){
		...
		$DataPembelianBarang = $this->retursupplierkonsinyasi_model->getRekapHarian($_GET);
		
		$html .= "<table>
					<tr>
						<td colspan=\"6\" align=\"center\">Rekap Harian Retur Supplier Konsinyasi</td>
					</tr>
					<tr>
		...
		$html .= "<table class=\"content\">
                    <thead>
                        <tr class=\"border_bottom\">
							...
							<th align=\"left\" width=\"150\">Supplier</th>
		...
		foreach ($DataPembelianBarang as $key => $value) {
			if($supplier == ""){
				$supplier = $value['supplier_kode'];
			}
			if($supplier != $value['supplier_kode']){
				$html .= "<tr class=\"border_top\">
							<td colspan=\"5\">Sub Total Per Supplier</td>
			...
			$html .= "<tr>
						...
						<td>".$value['tanggal']."</td>
						<td>".$value['bukti']."</td>
						<td width=\"80\">".$value['supplier_kode']."</td>
						<td width=\"150\">".$value['nama_supplier']."</td>
						<td align=\"right\">".number_format($value['jumlah'], 2)."</td>
					  </tr>";
			$inc++;
		...
		$DataKelompok = $this->retursupplierkonsinyasi_model->getRekapKelompok($_GET);
		$html .= "<table width=\"50%\">";
		$TotalAll = 0;
		foreach ($DataKelompok as $key => $value) {
			$html .= "<tr>
		...
		$pdf->AddPage();
		$pdf->writeHTML($html, true, false, true, false, '');
		$pdf->Output('RekapHarianReturSupplierKons.pdf', 'I');
	}
}