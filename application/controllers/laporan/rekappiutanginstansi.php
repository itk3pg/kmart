...
class Rekappiutanginstansi extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('laporan/rekappiutanginstansi');
		$this->load->view('general/footer');
	}
	
	public function getrekappiutanginstansi(){
		$DataRekapPiutangInstansi = $this->laporan_model->getRekapPiutangInstansi($_POST);
		...
		foreach ($DataRekapPiutangInstansi as $key => $value) {
			...
			$value['dpp'] = round($value['dpp'], 2);
			$value['ppn'] = round($value['ppn'], 2);
			$value['ftotal'] = round($value['ftotal'], 2);
			$html .= "<tr>
						<td>".$value['fdate']."</td>
						<td>".$value['fcode']."</td>
			...
			$TotalAll += $value['ftotal'];
			$TotalDPPAll += $value['dpp'];
			$TotalPPNAll += $value['ppn'];
			$TotalTotalAll += $value['ftotal'];
		}
		$html .= "<tr>
					<td colspan=\"6\"><strong>Sub Total :</strong></td>
					<td align=\"right\"><strong>".number_format($TotalPerbukti,2)."</strong></td>
		...
		$html .= "<tr>
					...
					<td align=\"right\"><strong>".number_format($TotalTotalAll,2)."</strong></td>
				  </tr>";
				 
		echo $html;
	}
	
	public function cetakrekappiutanginstansi(){
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment;Filename=rekap_piutang_instansi.xls");
		$DataRekapPiutangInstansi = $this->laporan_model->getRekapPiutangInstansi($_GET);
		
		$html = "<table>
					<tr>
						<td colspan=\"10\" align=\"center\">REKAP PIUTANG INSTANSI</td>
					</tr>
					<tr>
