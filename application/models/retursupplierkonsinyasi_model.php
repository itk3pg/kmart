...
class Retursupplierkonsinyasi_model extends CI_Model{
	function __construct(){
		parent::__construct();
	}
	
	function getDataReturSupplier($data){
		$query = "select a.bukti, a.tanggal, a.barang_kode, b.nama_barang, b.satuan, a.kwt, a.harga, a.ppn, a.jumlah, 
		a.supplier_kode, c.nama_supplier, a.keterangan, a.tukar_nota_bukti 
		from retur_supplier_konsinyasi a left join barang b on a.barang_kode=b.kode left join supplier c on a.supplier_kode=c.kode 
	...
	function getDataListReturSupplier($data){
		...
		from retur_supplier_konsinyasi a left join supplier c on a.supplier_kode=c.kode 
	...
	function SimpanReturSupplier($data){
		$query = "";
		if($data['mode'] == "i"){
			$query = "insert into retur_supplier_konsinyasi(bukti, tanggal, barang_kode, kwt, harga, ppn, jumlah, supplier_kode, keterangan, is_hapus, status, user_id, waktu_insert, waktu_update) 
		...
		// $querySelect = "select bukti, supplier_kode, tanggal, sum(jumlah) as jumlah from retur_supplier_konsinyasi where bukti='".$data['bukti']."' and supplier_kode='".$data['supplier_kode']."' and is_hapus='0' group by bukti, supplier_kode, tanggal";
	...
	function getDataBarangReturSupplier($data){
		$query = "select a.bukti, a.tanggal, a.barang_kode, b.nama_barang, b.satuan, a.kwt, a.harga, a.ppn, a.jumlah, a.supplier_kode, c.nama_supplier, a.keterangan 
		from retur_supplier_konsinyasi a left join barang b on a.barang_kode=b.kode left join supplier c on a.supplier_kode=c.kode  
	...
	function getRekapReturSupplier($data){
		...
		if(isset($data['supplier_kode'])){
			//$query = "select a.bukti, a.tanggal, a.supplier_kode, sum(a.jumlah) as jumlah from retur_supplier_konsinyasi a where a.supplier_kode='".$data['supplier_kode']."' and (a.tukar_nota_bukti is null or a.tukar_nota_bukti='') and a.is_hapus='0' group by a.bukti, a.supplier_kode order by a.tanggal desc";
			...
			//$query = "select a.bukti, a.tanggal, a.supplier_kode, sum(a.jumlah) as jumlah from retur_supplier_konsinyasi a where a.tukar_nota_bukti='".$data['tukar_nota_bukti']."' and a.is_hapus='0' group by a.bukti, a.supplier_kode order by a.tanggal desc";
	...
	function HapusReturSupplier($data){
		$query = "update retur_supplier_konsinyasi set is_hapus='1' where bukti='".$data['bukti']."'";
	...
	function setStatusReturSupplier($data){
		$query = "update retur_supplier_konsinyasi a set a.tukar_nota_bukti='".$data['tukar_nota_bukti']."' where a.bukti='".$data['retur_bukti']."' and a.supplier_kode='".$data['supplier_kode']."'";
		//echo $query."<br/>"; 
		$this->db->query($query);
	...
	function HapusStatusReturSupplier($data){
		$query = "update retur_supplier_konsinyasi set tukar_nota_bukti=NULL where tukar_nota_bukti='".$data['bukti']."'";
		$this->db->query($query);
	...
	function getAllDataRetur($data){
		$query = "select a.barang_kode, a.tanggal from retur_supplier_konsinyasi a where a.bukti='".$data['bukti']."'";
		$result = $this->db->query($query);
		
		return $result->result_array();
	}

	function getRekapHarian($data){
		$query = "select a.bukti, a.supplier_kode, c.nama_supplier, a.tanggal, sum(a.jumlah) as jumlah from retur_supplier_konsinyasi a left join supplier c on a.supplier_kode = c.kode where (date(a.tanggal) between '".$data['tanggal_awal']."' and '".$data['tanggal_akhir']."') and a.is_hapus='0' group by a.bukti, a.supplier_kode order by a.supplier_kode, a.tanggal asc";
		
		$result = $this->db->query($query);
		return $result->result_array();
	}

	function getRekapKelompok($data){
		$query = "select b.kategori, c.nama, sum(a.jumlah) as jumlah from retur_supplier_konsinyasi a left join barang b on a.barang_kode=b.kode left join kategori_barang c on b.kategori=c.kode where (date(a.tanggal) between '".$data['tanggal_awal']."' and '".$data['tanggal_akhir']."') and a.is_hapus='0' group by b.kategori";
