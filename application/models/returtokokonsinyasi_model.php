...
class Returtokokonsinyasi_model extends CI_Model{
	...
	function getDataReturToko($data){
		...
		from retur_toko_konsinyasi a left join barang b on a.barang_kode=b.kode left join toko c on a.toko_kode=c.kode 
	...
	function getDataListReturToko($data){
		...
		from retur_toko_konsinyasi a left join toko c on a.toko_kode=c.kode 
	...
	function SimpanReturToko($data){
		$query = "";
		if($data['mode'] == "i"){
			$query = "insert ignore into retur_toko_konsinyasi(bukti, tanggal, barang_kode, kwt, harga, jumlah, toko_kode, keterangan, is_approve, is_hapus, user_id, waktu_insert, waktu_update) 
			...
			$query = "insert into retur_toko_konsinyasi(bukti, tanggal, barang_kode, kwt, harga, jumlah, toko_kode, keterangan, is_approve, is_hapus, user_id, waktu_insert, waktu_update) 
			values('".$data['bukti']."', '".$data['tanggal']."', '".$data['barang_kode']."', ".$data['kwt'].", ".$data['harga'].", ".$data['jumlah'].", '".$data['toko_kode']."', 
	...
	function getDataBarangReturToko($data){
		...
		from retur_toko_konsinyasi a left join barang b on a.barang_kode=b.kode left join (select * from saldo_barang_konsinyasi where bulan=month(NOW()) and tahun=year(NOW())) c on a.toko_kode=c.toko_kode and a.barang_kode=c.barang_kode left join toko d on a.toko_kode=d.kode 
	...
	function HapusReturToko($data){
		$query = "update retur_toko_konsinyasi set is_hapus='1', waktu_update=NOW(), user_id='".$this->session->userdata('username')."' where bukti='".$data['bukti']."'";
	...
	function getDataBarangSupplier($data){
		...
		left join (select * from saldo_barang_konsinyasi where toko_kode='".$data['toko_kode']."' and bulan=".date('m')." and tahun=".date('Y').") c on a.kode=c.barang_kode 
	...
	function getSaldoBarangToko($data){
		$query = "select a.saldo_akhir_kwt from saldo_barang_konsinyasi a where a.toko_kode='".$data['toko_kode']."' 
	...
	function TerimaRetur($data){
		$query = "update retur_toko_konsinyasi set is_approve='1' where bukti='".$data['bukti']."'";
		
		$this->db->query($query);
	}

	function getDataTransferGudang($data){
		$query = "select * from transfer_toko_konsinyasi where ref_ot='".$data['bukti_rt']."' and is_hapus='0'";
	...
	function SimpanTransferKeToko($data){
		...
		$querytg = "insert into transfer_toko_konsinyasi(bukti, barang_kode, toko_kode, tanggal, kwt, harga, jumlah, ref_ot, is_approve, user_id, waktu_insert, waktu_update, is_hapus, urut) select '".$BuktiTG."' as bukti, barang_kode, '".$data['toko_kode']."' as toko_kode, '".$Param['tanggal']."' as tanggal, kwt, harga, jumlah, bukti as ref_ot, '0' as is_approve, user_id, NOW() as waktu_insert, NOW() as waktu_update, '0' as is_hapus, urut from retur_toko_konsinyasi where bukti='".$data['bukti_rt']."' and is_hapus='0'";
