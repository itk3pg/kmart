<div id="page-wrapper">
	<div class="row">
	    <div class="col-lg-12">
	        <h1 style="margin-top: 15px;" class="page-header">Master Kategori Menu</h1>
	    </div>
...
<script>
	...
	function InitializedList(){
		...
		var options = {
			...
			opener: {
				 active: true,
				 close: '/resto/images/Remove2.png',
				 open: '/resto/images/Add2.png',
				 openerCss: {
	...
	function getList(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>index.php/kategorimenu/getlistkategori",
			data: "",
	...
	function simpankategori(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>index.php/kategorimenu/simpankategori",
			data: "nama="+$("#nama_kategori").val()+"&mode="+$("#mode_form").val()+"&kode="+$("#kode_kategori").val(),
			success: function(msg){
				$("#nama_kategori").val("");
				$("#mode_form").val("i");
				$("#btn_cancel").hide();
	...
	function setParent(current, parent){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>index.php/kategorimenu/setparent",
	...
	function HapusKategori(kode){
		...
		if(conf){
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url(); ?>index.php/kategorimenu/hapuskategori",
				data: "kode="+kode,
				success: function(msg){
					getList();
				}
	...
	function EditKategori(kode, nama){
		$("#btn_cancel").show();
