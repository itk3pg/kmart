...
<div id="page-wrapper">
	<div class="row">
	    <div class="col-lg-12">
	        <h1 style="margin-top: 15px;" class="page-header">Retur Toko (RT)</h1>
	    </div>
	    <!-- /.col-lg-12 -->
	</div>
		...
		<div class="col-lg-12">
			<table style="float: right;">
				<tr>
					...
					<td>&nbsp;&nbsp;</td>
					<td valign="middle">
						<button id="btn_upload" onclick="CetakRekapReturToko()" class="btn btn-success" type="button">
					...
					<td>&nbsp;&nbsp;</td>
					<td valign="middle">
						<button id="btn_upload" onclick="CetakRekapReturTokoExcel()" class="btn btn-success" type="button">
							<i class="fa fa-print"></i>
							&nbsp;&nbsp;Rekap Harian (xls)
						</button>
					</td>
		...
		<div class="col-lg-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					...
					<?php } ?>
					<?php if($this->session->userdata('group_kode') == "GRU0000" || $this->session->userdata('group_kode') == "GRU0009"){ ?>
					<button id="btn_uedit" onclick="BatalTerimaReturToko()" class="btn btn-danger btn-sm" type="button">
						<i class="fa fa-check"></i>
						&nbsp;&nbsp;Batal Terima
					</button>
					<?php } ?>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
        			...
        			<h4 class="modal-title" id="myModalLabel">Form Retur Barang Toko (RT)</h4>
				</div>
...
<script>
	...
	function LoadDataReturToko(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/returtoko/getdatareturtoko",
	...
	function TambahBarangReturToko(satuan){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/returtoko/getsaldobarangtoko",
	...
	function SimpanReturToko(){
		...
		if(mode == "i"){
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url(); ?>index.php/returtoko/hapusreturtoko",
				data: "bukti="+bukti,
				success: function(msg){
	...
	function ajaxsimpanreturtoko(bukti, dataArr, index){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/returtoko/simpanreturtoko",
	...
	function openFormEditReturToko(){
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['is_approve'] == "1"){
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>index.php/returtoko/getdatabarangreturtoko",
	...
	function ListBarangSupplier(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/returtoko/getdatabarangsupplier",
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['is_approve'] == "1"){
				...
				if(ConfirmTerima){
					$.ajax({
						type: "POST",
						url: "<?= base_url() ?>index.php/returtoko/terimaretur",
						data: "bukti="+data['bukti']+"&toko_kode="+data['toko_kode'],
						success: function(msg){
							LoadDataReturToko();
						}
	...
	function CetakTransfer(){
		...
		if(typeof data_obj == "undefined"){
			...
			var data = json_decode(base64_decode(data_obj));
			window.open('<?= base_url('index.php/returtoko/cetakreturtoko?') ?>bukti='+data['bukti']+'&toko_kode='+data['toko_kode'],'_blank');
		}
	}
	...
	function CetakTransfer2(){
		...
		if(typeof data_obj == "undefined"){
			...
			var data = json_decode(base64_decode(data_obj));
			window.open('<?= base_url('index.php/returtoko/cetakreturtoko?') ?>bukti='+data['bukti']+'&toko_kode='+data['toko_kode']+'&cetak=2','_blank');
		}
	}
	...
	function CetakRekapReturToko(){
		var tanggal_awal = $("#search_tanggal_awal").val();
		var tanggal_akhir = $("#search_tanggal_akhir").val();
		var toko_kode = $("#returtoko_cari_toko").val();
		window.open('<?= base_url('index.php/returtoko/getrekapreturtoko?') ?>tanggal_awal='+tanggal_awal+'&tanggal_akhir='+tanggal_akhir+'&toko_kode='+toko_kode,'_blank');
	}

	function CetakRekapReturTokoExcel(){
		var tanggal_awal = $("#search_tanggal_awal").val();
		var tanggal_akhir = $("#search_tanggal_akhir").val();
		var toko_kode = $("#returtoko_cari_toko").val();
		window.open('<?= base_url('index.php/returtoko/getrekapreturtoko_xls?') ?>tanggal_awal='+tanggal_awal+'&tanggal_akhir='+tanggal_akhir+'&toko_kode='+toko_kode,'_blank');
	}
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['is_approve'] == "1"){
				...
				if(ConfirmTerima){
					$.ajax({
						type: "POST",
						url: "<?= base_url() ?>index.php/returtoko/batalterimareturtoko",
						data: "bukti="+data['bukti']+"&toko_kode="+data['toko_kode'],
						success: function(msg){
							LoadDataReturToko();
						}
				...
				alert("Retur toko belum dilakukan penerimaan");
			}
		}
	}
	...
	function SimpanTransferToko(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/returtoko/simpantransferketoko",
