...
<div id="page-wrapper">
	<div class="row">
	    <div class="col-lg-12">
	        <h1 style="margin-top: 15px;" class="page-header">Transfer Toko Konsinyasi (TN)</h1>
	    </div>
	    <!-- /.col-lg-12 -->
	</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table style="margin-bottom: 5px;">
						<tr>
							<td style="width: 125px;">
								<input style="width: 115px;" type="text" name="list_tn_tahun" id="list_tn_tahun" value="<?= date("Y"); ?>" class="form-control" />
							</td>
							<td style="width: 125px;">
								<select style="width: 115px;" class="form-control" name="list_tn_bulan" id="list_tn_bulan">
									<option <?php if(date('m') == '01') echo "selected=\"selected\"" ?> value="01">Januari</option>
									<option <?php if(date('m') == '02') echo "selected=\"selected\"" ?> value="02">Februari</option>
									<option <?php if(date('m') == '03') echo "selected=\"selected\"" ?> value="03">Maret</option>
									<option <?php if(date('m') == '04') echo "selected=\"selected\"" ?> value="04">April</option>
...
<script>
	...
	function LoadDataTG(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/transfergudangkonsinyasi/getdatatg",
	...
	function SimpanTG(){
		...
		if(mode == "i"){
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url(); ?>index.php/transfergudangkonsinyasi/hapustg",
	...
	function ajaxsimpantg(bukti, dataArr, index){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/transfergudangkonsinyasi/simpantg",
	...
	function openFormEditTG(){
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['is_approve'] == "1"){
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>index.php/transfergudangkonsinyasi/getdatabarangtg",
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['is_approve'] == "1"){
				...
				if(ConfirmTerima){
					$.ajax({
						type: "POST",
						url: "<?= base_url() ?>index.php/transfergudangkonsinyasi/terimatransfertoko",
						data: "bukti="+data['bukti'],
						success: function(msg){
							LoadDataTG();
						}
	...
	function CetakTransfer(){
		...
		if(typeof data_obj == "undefined"){
			...
			var data = json_decode(base64_decode(data_obj));
			window.open('<?= base_url('index.php/transfergudangkonsinyasi/cetaktg?') ?>bukti='+data['bukti'],'_blank');
		}
	}
	...
	function CetakTransferPlain(){
		...
		if(typeof data_obj == "undefined"){
			...
			var data = json_decode(base64_decode(data_obj));
			window.open('<?= base_url('index.php/transfergudangkonsinyasi/cetaktgplain?') ?>bukti='+data['bukti'],'_blank');
		}
	}
