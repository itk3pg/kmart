...
<div id="page-wrapper">
	<div class="row">
	    <div class="col-lg-12">
	        <h1 style="margin-top: 15px;" class="page-header">Retur Supplier (RS)</h1>
	    </div>
	    <!-- /.col-lg-12 -->
	</div>
		...
		<div class="col-lg-12">
			<table style="float: right;">
				<tr>
					...
					<td>&nbsp;&nbsp;</td>
					<td valign="middle">
						<button id="btn_upload" onclick="CetakRekapReturSupplier()" class="btn btn-success" type="button">
					...
					<td>&nbsp;&nbsp;</td>
					<td valign="middle">
						<button id="btn_upload" onclick="CetakRekapReturSupplierExcel()" class="btn btn-success" type="button">
							<i class="fa fa-print"></i>
							&nbsp;&nbsp;Rekap Harian (xls)
						</button>
					</td>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
        			...
        			<h4 class="modal-title" id="myModalLabel">Form Retur Barang Supplier (RS)</h4>
				</div>
...
<script>
	...
	function LoadDataReturSupplier(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/retursupplier/getdataretursupplier",
	...
	function SimpanReturSupplier(){
	    ...
	    $("#table-dummy-retursupplier td").each(function(){
	   	...
	   	if($("#retursupplier_supplier").select2("val") != "" && $("#retursupplier_supplier").select2("val") != null){
		    var mode = $("#mode").val();
			...
			if(mode == "i"){
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url(); ?>index.php/retursupplier/hapusretursupplier",
					data: "bukti="+bukti,
					success: function(msg){
	...
	function ajaxsimpanretursupplier(bukti, dataArr, index){
		...
		if(kwt > 0){
			$.ajax({
				type: "POST",
				url: "<?= base_url() ?>index.php/retursupplier/simpanretursupplier",
	...
	function syncstokretursupplier(bukti){
		$.ajax({
			type: "POST",
			url: "<?= base_url(); ?>index.php/retursupplier/syncstokretursupplier",
			data: "bukti="+bukti,
			success: function(msg){
				ShowMessage("success", "Stok berhasil diupdate");
			}
		});
	}
	...
	function openFormEditReturSupplier(){
		...
		if(typeof data_obj == "undefined"){
			...
			if(data['tukar_nota_bukti'] != ""){
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>index.php/retursupplier/getdatabarangretursupplier",
					data: "bukti="+data['bukti'],
					success: function(msg){
	...
	function ListBarangSupplier(){
		...
		$.ajax({
			type: "POST",
			url: "<?= base_url() ?>index.php/retursupplier/getdatabarangsupplier",
	...
	function CetakReturSupplier(){
		...
		if(typeof data_obj == "undefined"){
			...
			var data = json_decode(base64_decode(data_obj));
			window.open('<?= base_url('index.php/retursupplier/cetakretursupplier?') ?>bukti='+data['bukti'],'_blank');
		}
	}
	...
	function CetakRekapReturSupplier(){
		var tanggal_awal = $("#search_tanggal_awal").val();
		var tanggal_akhir = $("#search_tanggal_akhir").val();
		window.open('<?= base_url('index.php/retursupplier/getlaprekapretursupplier?') ?>tanggal_awal='+tanggal_awal+'&tanggal_akhir='+tanggal_akhir,'_blank');
	}

	function CetakRekapReturSupplierExcel(){
		var tanggal_awal = $("#search_tanggal_awal").val();
		var tanggal_akhir = $("#search_tanggal_akhir").val();
		window.open('<?= base_url('index.php/retursupplier/getlaprekapretursupplier_xls?') ?>tanggal_awal='+tanggal_awal+'&tanggal_akhir='+tanggal_akhir,'_blank');
	}
</script>