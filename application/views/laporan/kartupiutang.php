...
<script>
	...
	function LoadDataMutasi(){
		...
		if(bulan <= 7 && tahun <= 2015){
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url('index.php/laporan/kartupiutang/getkartupiutang') ?>",
				data: "bulan="+bulan+"&tahun="+tahun+"&toko_kode="+toko_kode,
				success: function(msg){
					$("#table-kartupiutang").html(msg);
	...
	function CetakDataMutasi(){
		...
		window.open('<?= base_url('index.php/laporan/kartupiutang/cetakkartupiutang?') ?>bulan='+bulan+'&tahun='+tahun+'&toko_kode='+toko_kode,'_blank');
	}
</script>